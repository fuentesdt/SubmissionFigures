...
true_val = [Kpl,VIF_scale_fact(1),kve,ve];
fit_param = {'k_P_L (sec^-^1)','VIF Scale (AU)','k_v_e (sec^-^1)','v_e'};
ylim_vect = [0.075,0.125;-1,5;-0.01,.05;.9,1.02];
for j = 1:size(fits,4)
figure('units','inches','Position',[6,2,6.5,3])
title_text = legend_labels;
for row = 1:numel(FAType)
    tmp_ax(row) = subplot(1,3,row);
    plot(N_vect,zeros(size(N_vect))+true_val(j),'g','LineWidth',line_width)
    hold on
    plot(N_vect,mean(fits(:,row,:,j),3),'color',line_color(row,:),'LineWidth',line_width)
    plot(N_vect,mean(fits(:,row,:,j),3)+std(fits(:,row,:,j)*1.96,[],3),...
    ...
    if row == 1
        ylabel(sprintf('Fit %s',fit_param{j}),...
            'FontSize',font_size,'FontWeight','bold')
    else
        %set(gca,'YTickLabel',[])
    end
