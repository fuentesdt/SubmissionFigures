...
for i = 1:numel(FAType)
    ...
    for k = 1:n_averages
        ...
        [x,~,allParams,resnorm(m,i,k),residual,exitflag,~,~,~]=model.fitData(...
            params,guessParams,TR_list,tmpMxy,'lb',0,'ub',10,'Y0',M0.');
        fits(m,i,k) = x;
        signals(m,i,k) = sum(sum(Mxy));
        lacSig(m,i,k) = sum(Mxy(2,:));
        pyrSig(m,i,k) = sum(Mxy(1,:));
    end
%         figure
%         plot(flipAngles.'*180/pi)
%         legend('lactate','pryvate')
%         xlabel('Time (sec)'),ylabel('Excitation Angle')
%         model.DataCompare(A,allParams,TRList,tmpMxy)
%         title(FAType{i})
%         drawnow
        toc       
end
